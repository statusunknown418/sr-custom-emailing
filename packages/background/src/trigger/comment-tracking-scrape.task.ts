 ...
 * lead magnet sequence, author the 3-message LinkedIn DM template, and persist
 * it to the D1 cache (`source = comment_tracking`).
 *
 * Flow: Apify scrape -> validate non-empty content -> Claude selects 3 distinct
 * magnets and writes the DM sequence -> POST the internal `post-cache/update`
 * endpoint. Fails loudly at every step so a post is never cached with empty
 * content or an invalid selection.
 */
